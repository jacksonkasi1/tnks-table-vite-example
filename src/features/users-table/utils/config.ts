...
import type {
  CaseFormatConfig,
} from "@/components/data-table/utils/case-utils";
...
export function useExportConfig() {
  ...
  const caseConfig: CaseFormatConfig = useMemo(
    () => ({
      ...
      apiFormat: "camelCase", // API parameters use camelCase (sortBy, pageSize)
  ...
  const transformFunction: DataTransformFunction<User> = useMemo(
    () => (row: User) => {
      const expenseAmount = parseFloat(row.totalExpenses) || 0;
      const currentYear = new Date().getFullYear();
      const joinYear = new Date(row.createdAt).getFullYear();

      return {
        ...row,
        // Format existing columns
        created_at: formatTimestampToReadable(row.createdAt),
        total_expenses: formatCurrency(row.totalExpenses),

        // Add NEW columns that don't exist in the original data
        status: row.expenseCount > 10 ? "ACTIVE" : "INACTIVE",
        expense_category:
          expenseAmount > 1000
            ? "HIGH_SPENDER"
            : expenseAmount > 500
